  ...
  state = {
	...
	setImgs = (imgNameArr)=>{
		...
		imgNameArr.forEach((imgName,index)=>{
			result.push({uid:-index,name:imgName,url:`${ServerIP}/upload/${imgName}`})
		})
  ...
  handleChange = async({file,fileList}) => {
    console.log(file,fileList)
		if(file.status === 'removed'){
			let {status,msg} = await axios.post('/product/img/delete',{name:file.name})
			if(status === 0){
  ...
  render() {
    ...
    return (
      <div className="clearfix">
        <Upload
					action="http://localhost:4000/product/img/upload"
					name="image"
